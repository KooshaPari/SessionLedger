...
| PLAN-P6 | partial | Coverage, property (incl. lifecycle FSM), fuzz, loom-lite race_model, soft loom smoke, enforced perf-budget, and enforced p95 latency gates landed; full permutation checkers remain | `tests/properties.rs`; `fuzz/`; `tests/race_model.rs`; `tests/loom_model.rs`; `.github/workflows/bench-gate.yml`; `docs/ops/perf-baseline.json` | Finish full loom/shuttle permutation checkers in WBS-6.2 | 2026-07-14 |
| PLAN-W8-B | done | Wave-34 result is 384/402 (95% A), meeting the A threshold | Independent audit-v38 result is at least 362/402 and >=90% | Hold A; pursue C06 SLSA-L3 + C11 signing + published brew/winget + C04 2FA for higher grades | 2026-07-17 |
