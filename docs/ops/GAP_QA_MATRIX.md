...
|---|---|---|---|---|---|
| C00 | 28/30 · partial | Soft SoftLatencyCheck + soft Miri + soft dhat alloc-profile landed; full loom/shuttle/blocking miri and jemalloc/production allocator remain | `audit/.lane-c00/C00.md`; `scripts/bench-gate.ps1`; `.github/workflows/miri-smoke.yml`; `docs/ops/alloc-profile.md`; `scripts/alloc-profile-check.ps1` | Enforce latency budgets; add full permutation checkers and jemalloc/production allocator | 2026-07-14 |
...
| C02 | 27/30 · partial | Crypto inventory + TLS deploy guidance landed; IdP/OAuth beyond shared key and KMS/at-rest remain | `docs/ops/crypto-inventory.md`; `scripts/crypto-inventory-check.ps1`; `crates/sl-daemon/src/resilience.rs`; `docs/ops/local-trust-boundary.md` | Add IdP/OAuth if remote multi-user deploy is needed; KMS/at-rest if scope expands | 2026-07-14 |
...
| C09 | 41/45 · partial | Expanded Cmd+K palette + live-daemon native attach evidence landed; native WebView parity gaps remain | `audit/.lane-c09/C09.md`; `crates/sl-viewer/src/command_palette.rs`; `scripts/record-native-webview-smoke.ps1`; `docs/a11y/status-regions-and-native-smoke.md` | Deepen native parity; palette discoverability polish | 2026-07-14 |
...
| PLAN-W8-B | done | Wave-29 result is 365/402 (91% A), meeting the B threshold | Independent audit-v38 result is at least 302/402 and >=75% | Hold A; pursue C11 signing + published brew/winget + C04 2FA for higher grades | 2026-07-14 |
