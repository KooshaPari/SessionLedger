...
| [`.github/workflows/ops-load.yml`](../../.github/workflows/ops-load.yml) | Weekly + `workflow_dispatch` | Heavier concurrent load against `/healthz`, `/readyz`, `/api/metrics`, and `/metrics` via [`scripts/load-smoke.ps1`](../../scripts/load-smoke.ps1). Soft `rss-budget` job also runs [`scripts/rss-budget-check.ps1`](../../scripts/rss-budget-check.ps1) against `POST /api/ingest` (see [`memory-budget.md`](memory-budget.md)). Soft `allocation-budget` job runs [`scripts/allocation-budget-check.ps1`](../../scripts/allocation-budget-check.ps1) (counting-allocator `process_session` smoke; see [`allocation-budget.md`](allocation-budget.md)). `pprof-smoke` job exercises the gated loopback profiling surface via [`scripts/pprof-smoke.ps1`](../../scripts/pprof-smoke.ps1). |
