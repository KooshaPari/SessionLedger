...
|---|---|---|---|---|---|
| C00 | 28/30 · partial | Soft SoftLatencyCheck + soft Miri landed; full loom/shuttle/blocking miri and jemalloc/dhat remain | `audit/.lane-c00/C00.md`; `scripts/bench-gate.ps1`; `.github/workflows/miri-smoke.yml`; `docs/ops/perf-baseline.json` | Enforce latency budgets; add full permutation checkers and jemalloc/dhat | 2026-07-14 |
...
| C02 | 26/30 · partial | API circuit breaker + rate limit landed; IdP/OAuth beyond shared key remains | `crates/sl-daemon/src/resilience.rs`; `crates/sl-daemon/src/http.rs`; `docs/ops/local-trust-boundary.md` | Add IdP/OAuth if remote multi-user deploy is needed | 2026-07-14 |
...
| C07 | 27/30 · partial | Exotic aarch64/musl cargo check + enforced perf-budget landed; mutation breadth remains | `tests/properties.rs`; `.github/workflows/cross-platform-build.yml`; `docs/ops/cross-platform-ci.md`; `.github/workflows/bench-gate.yml` | Expand mutation/property depth beyond exotic matrix | 2026-07-14 |
| C08 | 25/30 · partial | Twenty-fixture corpus + compression eval gate landed; token-burn ledger and cross-lang remain shallow | `audit/.lane-c08/C08.md`; `docs/ops/eval-manifest.json`; `tests/compression_eval.rs`; `.github/workflows/bench-gate.yml` | Grow production-scale families; cross-language adapters | 2026-07-14 |
| C09 | 40/45 · partial | Cmd+K palette + live-daemon native attach evidence landed; broader palette actions remain | `audit/.lane-c09/C09.md`; `crates/sl-viewer/src/command_palette.rs`; `scripts/record-native-webview-smoke.ps1`; `docs/a11y/status-regions-and-native-smoke.md` | Expand palette actions; deepen native parity | 2026-07-14 |
...
| C11 | 37/45 · partial | Brew/winget fill script + reverse-proxy + unsigned MSI/PKG landed; live tap/winget publish and platform signing remain | `audit/.lane-c11/C11.md`; `scripts/fill-packaging-checksums.ps1`; `docs/ops/brew-winget-publish.md`; `.github/workflows/release.yml` | Publish brew/winget; add credentials + signed production installer evidence | 2026-07-14 |
