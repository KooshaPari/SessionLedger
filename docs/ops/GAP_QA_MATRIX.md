...
|---|---|---|---|---|---|
| C00 | 24/30 · partial | Production SQLite MemoryStore adapter, cancellation, profiling/race/RSS gates | `audit/.lane-c00/C00.md`; `src/schema/`; `docs/ops/schema-migrations.md`; `scripts/openapi-drift-check.ps1` | Wire durable adapter on top of migration scaffold | 2026-07-13 |
...
| C07 | 25/30 · partial | PR race-smoke OS matrix landed; durable flake tracking and default PR cross-platform build-test remain incomplete | `tests/race_smoke.rs`; `.github/workflows/race-smoke.yml`; `docs/ops/cross-platform-ci.md`; `tests/properties.rs`; `fuzz/` | Extend WBS-6.2 with enforced perf-budget evidence | 2026-07-13 |
...
| C10 | 33/36 · partial | Stream/replay panes still use LoadingState; broader empty/error golden coverage remains | `audit/.lane-c10/C10.md`; `crates/sl-viewer/src/async_states.rs`; `tests/visual/golden/l1-content-skeleton.png`; `tests/visual/harness/visual.spec.js` | Adopt ContentSkeleton on remaining async panes | 2026-07-13 |
...
| PLAN-W8-B | done | Wave-18 result is 326/402 (81% B), meeting the B threshold | Independent audit-v38 result is at least 302/402 and >=75% | Hold B; pursue C11 signing implementation for higher grades | 2026-07-13 |
