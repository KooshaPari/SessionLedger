...
|---|---|---|---|---|---|
| C00 | 29/30 · partial | Blocking loom/shuttle/Miri/TSan permutation SelfCheck landed; daemon-graph ports and jemalloc/production allocator remain | `audit/.lane-c00/C00.md`; `scripts/loom-permutation-check.ps1`; `scripts/shuttle-permutation-check.ps1`; `scripts/miri-permutation-check.ps1`; `scripts/tsan-permutation-check.ps1`; `.github/workflows/tsan-permutation.yml` | Add broadcast/SSE daemon-graph permutation ports; promote jemalloc default-on | 2026-07-17 |
| C01 | 30/30 · done | Quality-gate SHA pin + clap completions + JSON/`es` i18n + Fluent `.ftl` catalog stub landed | `audit/.lane-c01/C01.md`; `locales/en.ftl`; `locales/es.ftl`; `src/i18n_fluent.rs`; `scripts/fluent-i18n-check.ps1`; `scripts/i18n-check.ps1` | Migrate viewer/CLI production strings through Fluent catalogs | 2026-07-17 |
...
| C04 | 27/30 · partial | CVE feed GHSA+OSV+NVD SSOT + blocking sandbox-boundary + hard rootless/no-net CI evidence + Renovate + branch-protection verify landed; maintainer 2FA and live runner matrix remain | `docs/ops/cve-feed-subscription.md`; `scripts/sandbox-boundary-check.ps1`; `scripts/rootless-nonet-check.ps1`; `.github/workflows/rootless-nonet.yml`; `scripts/branch-protection-check.ps1`; `docs/ops/maintainer-2fa.md` | Human records 2FA; add live rootless-only runner matrix + cargo-fetch no-net | 2026-07-17 |
...
| C06 | 30/30 · done | Unconditional release-blocking OCI cosign + source provenance + reusable hermetic workflow + protected-environment SLSA checklist landed; full L3 attestation remains | `scripts/oci-cosign-verify.ps1`; `docs/ops/source-provenance.md`; `.github/workflows/reusable-hermetic-build.yml`; `docs/ops/slsa-protected-environment.md`; `scripts/slsa-protected-env-check.ps1` | Close protected-environment SLSA Build L3 attestation | 2026-07-17 |
...
| PLAN-P6 | partial | Coverage, property (incl. lifecycle FSM), fuzz, loom-lite race_model, blocking loom/shuttle/Miri permutation, enforced perf-budget, and enforced p95 latency gates landed; TSan + daemon-graph permutation remain | `tests/properties.rs`; `fuzz/`; `tests/race_model.rs`; `tests/loom_model.rs`; `.github/workflows/loom-permutation.yml`; `.github/workflows/shuttle-permutation.yml`; `.github/workflows/miri-permutation.yml`; `.github/workflows/bench-gate.yml`; `docs/ops/perf-baseline.json` | Add TSan + broadcast/SSE daemon-graph permutation in WBS-6.2 | 2026-07-17 |
| PLAN-W8-B | done | Wave-37 result is 390/402 (97% A), meeting the A threshold | Independent audit-v38 result is at least 362/402 and >=90% | Hold A; pursue C11 signing + published brew/winget + human org 2FA attestation + full SLSA-L3 for higher grades | 2026-07-17 |
