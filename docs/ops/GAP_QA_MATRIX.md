...
|---|---|---|---|---|---|
| C00 | 24/30 · partial | Cancellation, profiling/race/RSS gates | `audit/.lane-c00/C00.md`; `crates/sl-daemon/src/main.rs`; `crates/sl-daemon/src/http.rs`; `src/ports/sqlite_memory.rs` | Add CancellationToken and formal concurrency checkers | 2026-07-13 |
...
| C02 | 23/30 · partial | Durable append-only audit sink landed; authenticated remote access and retention policy remain | `crates/sl-daemon/src/audit.rs`; `crates/sl-daemon/src/http.rs`; `docs/ops/local-trust-boundary.md` | Document audit retention and export policy | 2026-07-13 |
...
| C05 | 27/30 · partial | Scheduled chaos/load smoke landed; OTLP metrics, live alert routing, and game-day cadence remain | `audit/.lane-c05/C05.md`; `.github/workflows/ops-chaos-smoke.yml`; `scripts/ops-chaos-smoke.ps1` | Add game-day cadence and live alert routing | 2026-07-13 |
...
| C08 | 24/30 · partial | Eleven-fixture eval corpus + anchor manifest landed; compression/token and cross-language parity remain shallow | `audit/.lane-c08/C08.md`; `docs/ops/eval-manifest.json`; `docs/reference/conformance/README.md` | Grow cross-language adapters | 2026-07-13 |
| C09 | 35/45 · partial | In-app keyboard help overlay landed; native WebView/live-data, cognitive, and efficiency coverage remain incomplete | `audit/.lane-c09/C09.md`; `crates/sl-viewer/src/help_overlay.rs`; `tests/visual/harness/a11y.spec.js` | Add native integration evidence | 2026-07-13 |
| C10 | 35/36 · partial | First-run empty variant and remaining R2/R3 error goldens | `audit/.lane-c10/C10.md`; `tests/visual/golden/e2-history-empty.png`; `tests/visual/golden/e4-search-empty.png`; `tests/visual/golden/r1-search-error.png`; `tests/visual/harness/visual.spec.js` | Add first-run empty + R2/R3 error variant goldens | 2026-07-13 |
...
| PLAN-W8-B | done | Wave-21 result is 333/402 (83% B), meeting the B threshold | Independent audit-v38 result is at least 302/402 and >=75% | Hold B; pursue C11 signing implementation for higher grades | 2026-07-13 |
