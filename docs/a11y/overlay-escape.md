...
3. **Inline confirm** — dismisses the Search **Clear** `alertdialog` without wiping fields (`search_view.rs`).
...
| Search clear confirm | `search_view.rs` `onkeydown` (dismiss `clear_pending`) | `a11y.spec.js` Clear confirm Escape |
